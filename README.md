...
RTL & PERSIAN VERSION OF VUE BEAUTIFUL CHAT

`vue-persian-chat` provides an intercom-like chat window that can be included easily in any project for free. It provides no messaging facilities, only the view component.
...
## [Demo](https://mattmezza.github.io/vue-persian-chat/)
...
```
$ yarn add vue-persian-chat
```

## Example
```javascript
import Chat from 'vue-persian-chat'
Vue.use(Chat)
```
...
```
```javascript
import CloseIcon from 'vue-persian-chat/src/assets/close-icon.png'
import OpenIcon from 'vue-persian-chat/src/assets/logo-no-bg.svg'
import FileIcon from 'vue-persian-chat/src/assets/file.svg'
import CloseIconSvg from 'vue-persian-chat/src/assets/close.svg'
...
export default {
  ...
  data() {
    return {
      ...
      messageList: [
          { type: 'text', author: `me`, data: { text: `سلام` } },
          { type: 'text', author: `user1`, data: { text: `چه خبر؟` } }
...
`Launcher` is the only component needed to use vue-persian-chat. It will react dynamically to changes in messages. All new messages must be added via a change in props as shown in the example.
...
```
git clone git@github.com:justcodelife/vue-persian-chat.git
cd vue-persian-chat
...
This is the red variant. Please check [this file](https://github.com/justcodelife/vue-persian-chat/tree/master/demo/src/colors.js) for the list of variants shown in the demo page online.
